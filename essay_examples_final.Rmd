...
exemplars <- function(model, number, topic) {
  ...
  for (i in seq_along(topic)) {
    ...
    examples <- plotQuote(name,
      width = 100, maxwidth = 500, text.cex = 1,
...
exemplars(stm.model.12, 12, c(1:12)) # number is our model number, topic is list of topics
```
...
exemplars <- function(model, number, topic) {
  ...
  for (i in seq_along(topic)) {
    ...
    examples <- plotQuote(name,
      width = 100, maxwidth = 500, text.cex = .7,
      main = paste0("stm.model.", number, ": Topic ", i)
    )
...
exemplars(stm.pc.32, 32, c(1:32)) # number is our model number, topic is list of topics
```
...
top.essays(stm.pc.6, 6, identifiers, c(1:6), original)
top.essays(stm.pc.12, 12, identifiers, c(1:12), original)
top.essays(stm.pc.18, 18, identifiers, c(1:18), original)
top.essays(stm.pc.24, 24, identifiers, c(1:24), original)
top.essays(stm.pc.32, 32, identifiers, c(1:32), original)
```
