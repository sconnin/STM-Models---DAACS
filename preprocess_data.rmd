...
# Shared helpers: recode_gender(), recode_race(), EXCLUDED_RACE_VALUES
...
precleaner <- function(x) {
  x %>%
    ...
    na.omit(doc_id) %>%
    filter() %>%
    distinct(doc_id, .keep_all = TRUE) %>% # remove dups
...
# Re-enabled. This write was commented out, but three notebooks read the file it
# produces -- contaminant_removal_final.Rmd, essay_examples_final.Rmd and
# stm_analyses_final.Rmd all call read_csv("preclean_text.csv"). With the write
# disabled, none of them could run from a clean checkout.
#
# `preclean_text` is computed above either way, so persisting it changes no
# value used later in this file. It only stops the pipeline depending on a file
# nothing produces. See CODE_REVIEW.md R-1.
#
# The file holds essay text joined to identifiers and is IRB-protected;
# .gitignore excludes *.csv so it stays local.
write_csv(preclean_text, "preclean_text.csv")
...
The gender and race recoding now runs through `R/covariate_levels.R`, which
fails on an unrecognised value rather than assigning one. Two corrections there
change the modelling sample (`CODE_REVIEW.md` S1-6, S1-7):

- **Gender.** The institutions encode it three different ways -- WGU
  `Female`/`Male`, Excelsior `FEMALE`/`MALE`, Albany `F`/`M`. The previous
  `ifelse(gender %in% c("Female", "FEMALE"), "F", "M")` matched the first two
  and sent everything else to `"M"`, which relabelled **all 2,088 Albany women
  as men** -- 53% of that institution's sample. They are now recoded correctly.
- **Race.** WGU wrote `Black` where the others wrote `Black or African
  American`, and `Native Hawaiian` where Excelsior wrote `Native Hawaiian or
  Other Pacific Islander`, so each group occupied two levels depending on which
  institution recorded the student. The `str_replace(race, "Black,", ...)` that
  was meant to prevent this required a trailing comma no value carries at that
  point, so it never fired. Both groups are now merged.

The `separate()` call that split Albany's `"Asian, non-Hispanic"` values was
removed: `recode_race()` strips that qualifier itself, so the
institution-specific branch is no longer needed.
...
covariates <- function(x, y) {
  x %>%
    dplyr::select(c(daacs_id, age, gender, all_of(y), first_gen, on_time_term1)) %>%
    rename(race = all_of(y)) %>%
    # Compare the stripped form, not the raw string. Albany writes
    # "Unknown, non-Hispanic"; the raw value matches no entry in
    # EXCLUDED_RACE_VALUES, so filtering it directly would carry the row
    # through to recode_race() and halt the pipeline on a category that was
    # always meant to be dropped. The original achieved this by running
    # separate() before the filter. See race_base_value() in R/covariate_levels.R.
    filter(!race_base_value(race) %in% EXCLUDED_RACE_VALUES) %>%
    ...
    mutate(across(c(gender, race), as_factor))
}
...
# Essays below the unique-word floor.
#
# The previous check was `sum(n_distinct(clean_data$text) < 50)`, which does not
# do what its comment claimed (CODE_REVIEW.md S1-8). n_distinct() returns ONE
# number for the whole column -- around 8,000 -- so comparing it to 50 gives a
# single logical, and the sum is 0 or 1 regardless of how short any essay is. It
# would have reported "pass" if every essay were a single character.
#
# This reports rather than asserts, because the floor is NOT guaranteed of the
# final text. cleaner() counts unique words and filters at one point in its
# chain, but four more substitutions run afterwards (pattern17, 19, 20, 21).
# An essay can clear the threshold when it is measured and fall below it once
# those strip further text, so a non-zero count here is expected, not a failure.
# See CODE_REVIEW.md S1-9: moving the filter to the end of the chain would drop
# additional essays and change the corpus, which is a modelling decision rather
# than a QA fix.
n_unique_words <- lengths(lapply(strsplit(clean_data$text, " "), unique))
n_short_essays <- sum(n_unique_words < 50)

cat(
  "essays below the 50-unique-word floor in the FINAL text:",
  n_short_essays, "of", nrow(clean_data),
  sprintf("(%.2f%%)\n", 100 * n_short_essays / nrow(clean_data))
)
cat("  fewest unique words in any retained essay:", min(n_unique_words), "\n")
```
