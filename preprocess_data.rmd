...
#
# cleaner() already enforces the >= 50 unique-word rule, so this is an assertion
# that the filter did its job, not a filter in its own right.
n_short_essays <- sum(
  lengths(lapply(strsplit(clean_data$text, " "), unique)) < 50
)
stopifnot(n_short_essays == 0)
cat("essays below the 50-unique-word floor:", n_short_essays, "\n")
```
