...
# One function replaces four near-identical blocks that differed only in which
# element of labelTopics() they read and what the caption said. `kable()` is
# knitr's, not kableExtra's.
topic_word_table <- function(label_metric, metric_name) {
  label_metric %>%
    as.data.frame() %>%
    ...
    knitr::kable(
      caption = paste0('Top Topic Words - "', metric_name, '": STM Prevalence Model')
    )
}

lab12.prob.stm <- topic_word_table(l12.d[1], "Probability")
lab12.frex.stm <- topic_word_table(l12.d[2], "FREX")
lab12.lift.stm <- topic_word_table(l12.d[3], "Lift")
lab12.score.stm <- topic_word_table(l12.d[4], "Score")
...
plot(first.gen.pred,
  # `Text.cex` (capital T) here was not a graphical parameter, so it was
  # ignored and R warned six times per plot. Every sibling plot uses `cex`.
  covariate = "first_gen", topics = c(1:12), cex = .25,
  model = stm.p.12, method = "difference",
...
plot(c2,
  ...
  printlegend = FALSE, main = paste0("Topic 2: ", topic_labels(topics = 2), " (P < 0.10)")
)
...
plot(c4,
  ...
  printlegend = FALSE, main = paste0("Topic 4: ", topic_labels(topics = 4), " (P < 0.10)")
)
...
plot(c6,
  ...
  printlegend = FALSE, main = paste0("Topic 6: ", topic_labels(topics = 6), " (P < 0.10)")
)
